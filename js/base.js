...
function find_product_detail()
{
    ...
    .done(function(reg) {
        ...
        var str = [];
        var pm;     //是否可以调整冷热0可以
        var numtatch = [];     //初始化口味条数
        var flavorId = [];  //初始化口味id
        var coff = [];  //初始化口味的英文
                   ...
                   '</div>'+
                    ...
                    '<div class="row" style="height: 7.3%;background:#000;line-height:53px;position:fixed;bottom:0;left:0;width: 100%;">'+
                        ...
                        '<div class="col-sm-6 col-xs-6 col-md-6 col-lg-6" style="text-align: right; height: 100%; ">'+
                            '<a href="javascript:void(0);" onclick="oder();" style="border:none; height: 100%; display: inline-block;"><img src="../image/lb/taste/_0019_OK.png" alt="" style="width:60%;margin-right:2.2%;" /></a>'+
                        '</div>'+
        ...
        // if (productId == 1)
        //     {
        //         $(".item_img").attr("src","../image/lb/Mocha.png");
        //     }
        // else{
        //         $(".item_img").attr("src","../" + getIten.img);
        //     };
...
function oder()
{
    ...
    var productId = getQueryString("productId");
    //料仓id
    var bunkerNumber_sugar = $(".bunkerNumber_sugar").val().split(',');
    //口味名称
    var _sugar = $("._sugar").val().split(',');
    //口味id
    var _flavorId = $("._flavorId").val().split(',');
    //口味进度条的数值数量
    var obj = "";
                ...
                obj += JSON.stringify(
                    {
                        ...
                        // obj +="{'flavorId':" + parseInt($('#sugar' + [i] +' #flavorId_sugar').val()) + ","
                        //         +"'flavorValue':" + parseInt($('#sugar' + [i] +' .progress-bar').text()) / 100+","
                        //            +"'bunkerNumber':" + parseInt($('#sugar' + [i] +' #bunkerNumber_sugar').val()) +"}";
                       // obj +="{flavorValue:0.5,bunkerNumber:" + parseInt(bunkerNumber_sugar[i]) +",flavorName:"+ _sugar[i] +"}";
    ...
    var DataJsons = encodeURI(DataJson);
...
function find_unpaid_order()
{
    ...
    if (orderid) {
        ...
        .done(function(reg) {
            ...
            // $("body").append('<div class="mask_layer"></div>');
            var str = [];
             var flshow = reg.flavorShow;

            //饮品的冷热
            if (reg.orderInfo.temperature == 1)
                {
                    reg.orderInfo.temperature = "热饮";
                }else
                {
                    reg.orderInfo.temperature = "冷饮";
                }


            str +='<p style="font-size: 2rem; margin-bottom: 5%; font-weight: bold;">'+ reg.orderInfo.productName.split(',')[0] +'</p>';

            for(var key in flshow)
                    {
                      str +='<p style="font-size: 1.5rem; margin-bottom: 3%; position:relative;"><span>'+ key.split(',')[0] +'</span><span style="position:absolute;right: 0;">'+ flshow[key] +'</span></p>';
                    };
                $("#product").html(str);
